...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[100dvh] w-full flex bg-slate-50 overflow-hidden relative">
      
      {/* ------------------------------------------------------------- */}
      {/* DESKTOP: Left side (Animated Orbital Background) */}
      {/* ------------------------------------------------------------- */}
      ...
      {/* ------------------------------------------------------------- */}
      {/* MOBILE: Dark Graphic Background */}
      {/* ------------------------------------------------------------- */}
      <div className="absolute inset-0 lg:hidden bg-gradient-to-br from-[#003835] via-[#004d4a] to-[#001f1e] overflow-hidden -z-10 flex flex-col items-center pt-[15dvh]">
        {/* Subtle glow blobs */}
        <div className="absolute top-10 left-[-20%] w-[60%] h-[40%] bg-[#009e99] rounded-full mix-blend-screen filter blur-[80px] animate-pulse" style={{ animationDuration: '8s' }}></div>
        <div className="absolute top-32 right-[-20%] w-[60%] h-[40%] bg-[#ccedea] rounded-full mix-blend-screen filter blur-[100px] animate-pulse" style={{ animationDuration: '10s' }}></div>

        <div className="relative z-10 flex flex-col items-center px-6 text-center">
          <div className="w-20 h-20 bg-white/10 border border-white/20 backdrop-blur-md rounded-3xl flex items-center justify-center shadow-2xl mb-6">
            {logoUrl ? (
              /* eslint-disable-next-line @next/next/no-img-element */
              <img src={logoUrl} alt="Logo" className="w-12 h-12 object-contain filter drop-shadow-md" />
            ) : (
              <Building2 className="w-10 h-10 text-white" />
            )}
          </div>
          <h2 className="text-4xl font-extrabold text-white tracking-tight mb-2 drop-shadow-lg">
            {appName}
          </h2>
          <p className="text-teal-100/80 text-base max-w-[260px]">
      ...
      {/* ------------------------------------------------------------- */}
      {/* Right side / Mobile Bottom Sheet: Login Form */}
      {/* ------------------------------------------------------------- */}
      <div className="w-full lg:w-1/2 flex flex-col justify-end lg:justify-center mt-auto lg:mt-0 relative z-10">
        
        {/* The Form Container */}
        <div className="w-full bg-white lg:bg-transparent rounded-t-[40px] lg:rounded-none px-6 sm:px-12 lg:px-24 pt-10 pb-12 shadow-[0_-15px_40px_-15px_rgba(0,0,0,0.15)] lg:shadow-none min-h-[45dvh] lg:min-h-0 flex flex-col justify-center">
          ...
          <div className="w-full max-w-md mx-auto lg:mx-0">
            ...
            <div className="lg:bg-white lg:py-10 lg:px-10 lg:shadow-2xl lg:shadow-slate-200/50 lg:rounded-3xl lg:border lg:border-slate-100 relative overflow-hidden">
              
              {/* Decorative corners (Desktop only) */}
              <div className="hidden lg:block absolute -top-10 -right-10 w-32 h-32 bg-teal-50 rounded-full blur-3xl opacity-60"></div>
              <div className="hidden lg:block absolute -bottom-10 -left-10 w-32 h-32 bg-emerald-50 rounded-full blur-3xl opacity-60"></div>

              <div className="relative z-10">
                <button
                  onClick={handleGoogleLogin}
                  disabled={isLoading}
                  className="w-full group flex items-center justify-center gap-4 py-4 px-4 border border-slate-200 rounded-2xl shadow-xl shadow-slate-200/40 text-base font-bold text-slate-800 bg-white hover:bg-slate-50 hover:border-slate-300 focus:outline-none focus:ring-4 focus:ring-teal-500/20 transition-all active:scale-[0.98] lg:active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed min-h-[60px]"
                >
                  <div className="bg-white p-1.5 rounded-full shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
                ...
                <div className="mt-8 relative">
                  ...
                  <div className="relative flex justify-center">
                    <span className="px-4 bg-white text-[11px] text-slate-400 font-bold tracking-wider uppercase">
                      Bảo mật nội bộ
                    </span>
                  </div>
                </div>

                <p className="mt-6 text-center text-[13px] text-slate-500 leading-relaxed px-4">
